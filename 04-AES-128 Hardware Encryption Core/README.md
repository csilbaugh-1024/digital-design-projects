...
After the initial key matrix is built, KeyExpansion expands this matrix into a larger one called the "expanded key array", which has four rows and $N_b(N_r + 1)$ columns, where $N_b$ represents the number of bits in the plaintext divided by 32, and $N_r$ represents the number of rounds. Since this project uses AES-128, $N_b$ is equal to 128 divided by 32, which is 4, and there are 10 rounds. So, $N_b(N_r + 1)$ is equal to 4 times 11, which is 44. So, the expanded key array is a matrix that has four rows and 44 columns, resulting in a total 176 entries. Because the expanded key array houses all 11 round keys to be used in the encryption process, this configuration can alternatively be viewed as 11 separate key arrays whose columns are attached to each other, one array per instance of AddRoundKey. The first array is to be used for the first instance of AddRoundKey, the second for the second instance of AddRoundKey, and so on. 
...
- include W rephrasing of matrix to introduce recursion formula

The leftmost 4 columns of the 44-column expanded key array contain the original, first key, and the next 10 round keys generated by KeyExpansion are all derived from this first key using a specific formula. 
