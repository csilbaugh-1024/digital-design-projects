...
## VHDL Design
Now that the full transmitter is sketched, the next step is to design it in VHDL. For large and complicated designs like these, it is best to design hierarchically and in steps. So, I started by desining the datapath, then the controller, then the baud rate controller, which synchronizes the transmitter to the desired baud rate. I decided to use 9600 baud, one of the most common baud rates.

### Datapath
Writing the VHDL for the datapath was very similar to sketching it. Save the three registers, I simply made a design source for each component in the datapath. Specifically, I made a comparator design source, an incrementer, a design source for each mux (the I mux, the W mux, and the y mux), and the right shifter. Then, I assembled these components hierarchically into a file called "Datapath.vhd", where I also introduced the synchronous clock and register behavior. For datapaths, it is generally better to keep registers at the top-level design, where the clock is introduced, rather than giving each register its own component to be used for hierarchical design. Finally, each component I included in the datapath, as well as the datapath itself, got its own testbench file so that it could be individually simulated and verified.

In this repository, I also included an unused "Edge_detector.vhd" in the datapath folder. Instead of using p1', my initial solution was to connect data_in to an edge detector and an inverter before reaching i1 of the y_mux. The edge detector would output a 1 for exactly one clock cycle the moment data_in goes HIGH. Any other time, it would output a 0. Passed through an inverter, this achieves the intended functionality of idly sending 1s until data_in goes HIGH to indicate a word is going to be transmitted. At this point, the 0 start bit would be sent, and the edge detector-inverter system would subsequently return to sending idle 1s. I chose to use this edge detector and inverter design because, initially, I had data_in connected directly to i1 of the y mux. However, there are two glaring problems with this design. First, if a user were to hold data_in HIGH for more than one clock cycle, the system would not work correctly because the 0 start bit would not be sent at the correct time. Because human motor control is so much slower than the FPGA's clock, this would be impossible to avoid. Second, it is generally bad practice to have one of the device's inputs connected directly to the datapath. Instead, datapaths should usually be controlled completely by the controller rather than directly by a user. 

As for the edge detector, it failed when I began to simulate with testbenches. I included it as part of the controller for more straightforward testing and, because the edge detector is a sequential circuit with its own FSM and controller, testing the edge detector meant that I was testing an FSM inside of another FSM. So, it did not function properly because it was a clock cycle behind. Furthermore, it was unncesessary hardware complexity compared to the very simple and straightforward p1' solution. So, after I discovered this solution, I disconnected the unnecessary edge detector. However, I have kept it here to help fully document my engineering design process, even though it was a mistake. After correcting the edge detector problem, the datapath passed all of the tests in its testbench and thus worked as intended.
...
This code splits the input data_in into data_in_prev and data_in_rise, which is used to observe whether data_in rises from 0 to 1. Physically, this would mean that flipping the switch to send a 7-bit ASCII word only sends it once. Without an edge detector, flipping the data_in switch HIGH would cause data to send multiple times because the FPGA's clock and the UART's baud rate move so much more quickly than human inputs do. Next, after I designed the baud rate generator, I came back to this controller code and included the baud rate where necessary. 

### Baud Rate Controller
With the datapath and controller fully designed, I next began the baud rate generator. Because this component only has to convert the FPGA's clock frequency to a 9600 baud rate, it required very little VHDL. Specifically, the baud rate generator sends a baud tick every 10,416 rising edges of the FPGA's 100 MHz internal clock. This baud generator will be used to control the rate at which the transmitter's controller changes state, and it will also be used in the receiver to ensure that it is synchronized with the transmitter. After verifying correct behavior with a testbench, the baud rate generator was complete.

### Full Transmitter Design
Now that the datapath, controller, and baud rate generator are all finished, they can be assembled together to create the full transmitter design. I did this in a file called "Transmitter_7bit.vhd". Because all of these components are already fully developed, all that remains is to design the connections between them as well as the inputs and outputs of the full transmitter. So, this took very little VHDL. This is one of the biggest strengths of hierarchical design.

After designing the transmitter, I began writing the testbench to verify its functionality. Though its VHDL is much larger than the surface-level VHDL of the transmitter, this testbench is very straightforward conceptually. I designed it to follow the valid behavior of the HLSM and the possible paths a user could take. For example, I programmed the testbench to verify correct behavior of the transmitter starting in IDLE with data_in LOW, and then it checks that the transmitter behaves as intended after one baud cycle after data_in goes HIGH, and so on. In other words, the testbench tests the different possible valid paths a user could take through the transmitter's HLSM as they use it. Once the testbench yielded perfectly successful results, I designed the constraints file to prepare to flash my design onto my FPGA. I mapped each of the 7 bits of W onto the rightmost 7 switches of my FPGA with the LSB on the single rightmost switch, and I mapped data_in to the very leftmost switch. After this, the transmitter was ready to be flashed onto the FPGA and tested physically.

## Testing on FPGA, Tutorial for Use
Testing on the FPGA was successful. I programmed my FPGA, connected it to my computer, and opened a PuTTY terminal at 9600 baud with 7 bits. Then, I verified with different ASCII letters that the transmitter worked as intended. Below, I have included a link to a YouTube video where I demonstrate how to use this UART transmitter.

https://youtube.com/shorts/aW5O3akN7ps?feature=share
